...
import HeaderTitle from 'src/components/dashboard/shared/HeaderTitle';

import ExtratoForm from 'src/components/dashboard/forms/search/ExtratoForm';
import FungoForm from 'src/components/dashboard/forms/search/FungoForm';
import MicotecaForm from 'src/components/dashboard/forms/search/MicotecaForm';
import AllForm from 'src/components/dashboard/forms/search/AllForm';

import * as shared from 'src/services/shared'
import { API_PATHS, get_all } from 'src/services/base'

import FungusCard from 'src/components/shared/FungusCard';
import MycoCard from 'src/components/shared/MycoCard';
import PlantCard from 'src/components/shared/PlantCard';
...
class AdminSearchView extends React.Component {
  ...
  deleteCallback = (id, state_type, all) => {
    ...
    if (state_type === "myco") {
      if (all) {
        this.setState({ result_all_m: s.result_all_m.filter(filter_item) });
      } else {
        this.setState({ result_myco: s.result_myco.filter(filter_item) });
      }
    } else if (state_type === "fungi") {
      if (all) {
        this.setState({ result_all_f: s.result_all_f.filter(filter_item) });
      } else {
        this.setState({ result_fungus: s.result_fungus.filter(filter_item) });
      }
    } else if (state_type === "plant") {
      if (all) {
        this.setState({ result_all_e: s.result_all_e.filter(filter_item) });
      } else {
        this.setState({ result_extract: s.result_extract.filter(filter_item) });
      }
    }
